...
def main(
    ...
    parquet_files = []
    for root, _dirs, files in os.walk(parquet_directory):
        for file in files:
            if file.endswith(".parquet"):
                parquet_files.append(os.path.join(root, file))
    if dist.get_rank() == 0:
        print("Loading from tables", parquet_files)
    ...
    print_example_kjt = True
    for (dense_features, labels, kjt) in it:

        if print_example_kjt:
            print("Example sample KJT input", kjt)
            print_example_kjt = False
